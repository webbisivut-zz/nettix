...
class PluginUpdateChecker_2_3 {
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, 'Tarkista päivitykset');
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = 'Lisäosa on uusin versio.';
			} else if ( $status == 'update_available' ) {
				$message = 'Uusi versio lisäosasta saatavilla.';
			} else {
				$message = sprintf('Päivityksen tilaa ei pystytty tarkistamaan:  "%s"', htmlentities($status));
			}
			printf(
